...
class ThirdStepViewController: BaseStepViewController {
    ...
    let numberInputView = SimpleInputView(viewModel: .init(textFieldViewModel: .init(placeholder: "관심있는 분야를 적어주세요")))
    let textView: BaseTextView = {
        return $0
    }(BaseTextView(viewModel: .init(placeholder: "자신있게 자기소개를 적어주세요.")))
    func setUI() {
        scrollView.addSubview(contentView)
